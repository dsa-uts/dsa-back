...
def get_evaluation_item(
    ...
    return schemas.EvaluationItemRecord(**evaluation_item.__dict__) if evaluation_item is not None else None


def get_test_case_list(
    db: Session, eval_id: str
) -> list[schemas.TestCaseRecord]:
    """
    特定の評価項目に基づくテストケースのリストを取得する関数
    """
    test_case_list = (
        db.query(models.TestCases)
        .filter(models.TestCases.eval_id == eval_id)
        .all()
    )
    return [schemas.TestCaseRecord(**test_case.__dict__) for test_case in test_case_list]
...
def get_problem_recursive(
    ...
    problem_record.evaluation_item_list = get_evaluation_item_list(db, lecture_id, assignment_id, for_evaluation)
...
def get_arranged_filepaths(
    ...
    return [schemas.ArrangedFileRecord(**arranged_file.__dict__) for arranged_file in arranged_files]
